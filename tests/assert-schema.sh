...
if [ "${1:-full}" = "database" ]; then
    ...
    assert_count "Nuvio migrations" 10 "$(query "select count(*) from nuvio_migrations.schema_migrations")"
    ...
    assert_count "authenticated client RPCs" 10 "$(query "select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname = any(array['list_my_sessions','register_current_device','register_current_session','revoke_my_session','sync_delete_library_items','sync_get_library_delta_cursor','sync_pull_library_delta','sync_push_library_items','sync_push_profile_settings_blob_guarded','sync_seed_provider_credentials']) and has_function_privilege('authenticated', p.oid, 'EXECUTE')")"
